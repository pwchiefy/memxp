...
1. **Credentials at rest** -- All credential data is stored in a SQLCipher-encrypted database (AES-256-CBC, page-level encryption). SQLCipher is the single encryption layer for data at rest.
...
| T5 | **MCP tool injection (malicious prompt)** | Vault is locked by default if `VAULT_MCP_SESSION_TOKEN` is set -- requires explicit authentication. Operator mode required for destructive operations (`vault_delete`, `vault_use`, `vault_expand`). Operator auto-promotion is disabled by default -- requires `VAULT_OPERATOR_AUTO_PROMOTE=1` env var, otherwise agent must provide passphrase explicitly. Installer does not pre-approve MCP tools by default (`--auto-approve` is opt-in). Reserved path prefixes (`_agents/`) prevent metadata tampering. Values are masked in search/list responses. `recall` supports `redact=true` to keep secrets out of chat history. | An agent with an active MCP session can read any credential via `recall(path, include_value=true)`. A compromised or manipulated agent could exfiltrate all credentials within a session. In the guided Claude install flow, `~/.mcp.json` may contain `VAULT_PASSPHRASE` for non-interactive unlocks, so local config disclosure also expands what a malicious local agent process can do. The `redact` and `copy_to_clipboard` modes mitigate but do not eliminate this if the agent bypasses them. |
| T6 | **Web UI session hijack** | Bound to `127.0.0.1` only (never `0.0.0.0`). Cookie-only auth -- session delivered exclusively via `Set-Cookie: vault_session=...; HttpOnly; SameSite=Strict` (no query-parameter fallback, no session ID in JSON responses). Session IDs are UUIDv4 (128-bit random). Idle timeout: 30 minutes. Max session lifetime: 24 hours. Strict CORS origin matching to `localhost:port`. Rate limiting on auth endpoints. | No HTTPS on localhost (plaintext HTTP). A local attacker or malware with access to loopback traffic could intercept session cookies. In-memory auth state (password hash, TOTP secret, sessions) is lost on restart -- there is no persistent web credential store. |
| T7 | **Memory dump / core dump exposure** | File permissions set to 0o600/0o700 on Unix (challenge files, database, config). Audit log machine IDs are SHA-256 hashed by default to reduce correlation. | The vault passphrase is held in process memory as a plain `String` during active use (passed to SQLCipher via `PRAGMA key`). Decrypted credential values also exist in memory. A memory dump (`/proc/pid/mem`, core file, swap) could expose them. No memory locking (`mlock`) is used. No zeroization is performed on passphrase or credential values. |
| T8 | **Supply chain attack (dependencies)** | Pinned dependency versions in `Cargo.lock`. Minimal dependency surface: `sha2`, `hmac` from RustCrypto (widely audited). `argon2` in `vault-web` for password hashing. `rusqlite` with `bundled-sqlcipher-vendored-openssl` (statically linked, no system OpenSSL). `rustls` (no OpenSSL for TLS). | The `rmcp` MCP framework, `keyring`, `axum`, and `tower-http` crates are less audited than the core cryptographic dependencies. A compromised upstream crate update could introduce backdoors. No `cargo-audit` or `cargo-vet` is currently integrated into CI. |
...
The `derive_sync_hmac_key` function derives a separate SHA-256-based key for P2P sync frame authentication. This key is distinct from the PBKDF2-derived SQLCipher encryption key and serves only to authenticate wire-protocol frames.

This derivation is intentionally fast (SHA-256, not Argon2) because:
1. It runs on every sync frame (latency-sensitive)
2. The passphrase has already been validated by SQLCipher (PBKDF2-HMAC-SHA512) during database open
